  ...
  export class DbService {
    ...
    listAllProducts() {
      return this.http.get(`${this.localUrl}/get/all/product/api`);
    }
    ...
    fetchingSingleOrder() {
      return this.http.get(
        `${this.localUrl}/list/single/order/api/`,
        this.authHeader()
      );
    }

    //reset passwords services

    //send request
    requestResetPassword(email: string) {
      return this.http.post(`${this.localUrl}/send/reset/password/api`, {
        email,
      });
    }

    //verify request via tokenisation
    verifyToken(token: string) {
      return this.http.get(`${this.localUrl}/reset/password/verify/${token}`);
    }

    //trigger resets
    resetPassword(token: string, newPassword: string) {
      return this.http.post(`${this.localUrl}/reset/password/${token}`, {
        newPassword,
      });
    }
  }
